...
class wcexd_checkout_fields {
	...
	public function set_custom_fields($fields) {
    	...
    	if(get_option('wcexd_company_invoice') && get_option('wcexd_private_invoice')) {
			$fields['billing']['billing_wcexd_invoice_type'] = array(
				'type'    => 'select',
				'options' => array(
					'private' => __('Privato', 'wcexd'),
					'company' => __('Azienda', 'wcexd')
				),
				'label'   => __('Tipologia fattura', 'wcexd'),
				'required'	  => true,
				'class'   => array(	
	...
	public function checkout_fields_check() {
		...
		if(get_option('wcexd_fields_check')) {
		    ...
		    /*Partita IVA*/
	 		if(isset($_POST['billing_wcexd_invoice_type']) && $_POST['billing_wcexd_invoice_type'] === 'company') {
