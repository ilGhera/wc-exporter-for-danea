...
<div id="wcexd-clienti" class="wcexd-admin">
	...
	<?php $clients_val = get_option( 'wcexd-clients-role' ); ?>
	...
	<form name="wcexd-clients-submit" id="wcexd-clients-submit" class="wcexd-form"  method="post" action="">
				...
				<td>
					<select class="wcexd wcexd-users" name="wcexd-users" form="wcexd-clients-submit">
						...
						foreach ( $roles as $key => $value ) {
							echo '<option value="' . esc_attr( $key ) . '"' . ( $key === $clients_val ? ' selected="selected"' : '' ) . '> ' . esc_html__( $value, 'woocommerce' ) . '</option>';
						}
						?>
					</select>
		...
		<p class="submit">
			<input type="submit" name="download_csv" class="button-primary" value="<?php esc_html_e( 'Download customers list (.CSV)', 'wc-exporter-for-danea' ); ?>" />
		</p>
	</form>
</div>
